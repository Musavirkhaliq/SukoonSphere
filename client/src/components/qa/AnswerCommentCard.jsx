...
import AnswerCommnetReply from "./AnswerCommentReply";

const AnswerCommentCard = ({ comment, answerId, onCommentUpdate }) => {
  const { user } = useUser();
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await customFetch.patch(
        `/qa-section/question/answer/comments/${comment._id}/like`
      );
      onCommentUpdate();
    } catch (error) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      //    "/answer/comments/:id",
      await customFetch.patch(`/qa-section/answer/comments/${comment._id}`, {
        content: editContent,
      });
      setIsEditing(false);
      onCommentUpdate();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await customFetch.delete(
        `/qa-section/question/answer/comments/${comment._id}`
      );
      onCommentUpdate();
