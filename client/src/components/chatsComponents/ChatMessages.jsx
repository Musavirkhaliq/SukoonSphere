      ...
      {messages?.map((message, index) => {
        ...
        return (
          <div key={index} className={`flex items-end ${isOwnMessage ? "justify-end" : "justify-start"}`}>
            {!isOwnMessage && (
             message?.sender?.avatar ? <img src={message?.sender?.avatar} alt="avatar" className="w-8 h-8 self-start bg-gray-700 rounded-full flex items-center justify-center text-sm font-bold text-white mr-2" /> : <div className="w-8 h-8 self-start bg-gray-700 rounded-full flex items-center justify-center text-sm font-bold text-white mr-2">
                {message?.sender?.name?.charAt(0)}
              </div>
            )}
            <div
              className={`relative max-w-[75%] p-3 text-sm rounded-lg shadow-md ${
                isOwnMessage ? "bg-blue-500 text-white rounded-br-none" : "bg-gray-800 text-white rounded-bl-none"
              }`}
            >
              {!isOwnMessage && <p className="text-blue-400 font-semibold mb-1">{message?.sender?.name}</p>}
