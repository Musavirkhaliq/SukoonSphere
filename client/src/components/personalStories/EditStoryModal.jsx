...
import { FaImage, FaTimes, FaEdit, FaUserSecret } from "react-icons/fa";
import customFetch from "@/utils/customFetch";
import JoditEditor from "jodit-react";

const EditStoryModal = ({ story, onClose, onStoryUpdated }) => {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedImage, setSelectedImage] = useState(
    story.imageUrl ? { url: story.imageUrl } : null
  );
  const [title, setTitle] = useState(story.title || "");
  const [content, setContent] = useState(story.content || "");
  const [isAnonymous, setIsAnonymous] = useState(story.isAnonymous || false);
  ...
  useEffect(() => {
    ...
    setSelectedImage(story.imageUrl ? { url: story.imageUrl } : null);
  }, [story]);
  ...
  const removeImage = () => {
    if (selectedImage?.url && !story.imageUrl) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await customFetch.put(`/personal-stories/${story._id}`, formData, {
        headers: {
      ...
      if (response.status === 200) {
        toast.success("Personal story updated successfully!");
        if (onStoryUpdated) {
      ...
      toast.error(
        error?.response?.data?.msg || "An error occurred during story update."
      );
    } finally {
